type N = number
type F = Function
type S<T = string> = T extends string ? T : string & T
type A<T = unknown> = Array<T> | T[]
type O<K = string, V = unknown> = Record<S<K>, V> | { [key: S<K>]: V }
...
type StringWithSome<T extends string, C extends string> = T extends `${C}${infer R}` ? `${C}${StringWithSome<R, C>}` : T
const strToWords = (str: string): A<> => str.split(' ').filter(String)
const toArray = (v?: any): typeof v extends A ? typeof v : A<typeof v> => Array.isArray(v) ? v : [v]
const typeOf = (v?: any, arr?: any) => {
	const tp = typeof v
	return Array.isArray(arr) ? arr.every(el => typeof el === tp) : tp
}
...
type StringEvery<T extends S> = `${T}`
type StringSome<T extends S> = S & `${T}`
type StringWith<T extends S> = `${T}`
